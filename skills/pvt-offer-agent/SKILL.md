---
name: pvt-offer-agent
description: >-
  Área 02 · Oferta de Private Build OS: qué vendes y a qué precio. Construye contigo los
  10 bloques de una oferta que se vende sola y los deja escritos en la página 02 ·
  Oferta de tu Notion. Diagnostica qué bloque falta por el síntoma del ciclo, audita
  cualquier pieza contra tu oferta, arma el pitch personalizado para un lead desde su
  ficha y decide cambios de precio, promesa, entregable o límite. Úsala cuando digas
  "arma mi oferta", "qué vendo y a qué precio", "me dicen lo pienso", "me negocian el
  precio", "estoy entregando de más", "audita mi propuesta", "arma el pitch para
  [lead]", "prepárame la propuesta para X", "quiero subir el precio". NO conduce la
  llamada ni maneja objeciones (pvt-closing-agent), NO define a quién le vendes (pvt-
  avatar-agent) y NO escribe el contenido que la declara (pvt-content-agent).
---

# pvt-offer-agent — Oferta: qué vendes y a qué precio
...
2. Si el hub no existe o Notion no está conectado: dile que escriba
   **"arrancar private build os"** (skill `pvt-arsenal-agent`). Si prefiere seguir sin
...
2. **01-03, el marco:** promesa (una, con plazo) · filtro (a quién no le sirve;
   sale de 03 · Avatar o `pvt-avatar-agent`) · mecanismo (si no se dibuja, no existe).
...
5. **Cierra con la ficha de salida** (caso elegido y por qué, objeción probable
   para `pvt-closing-agent`, link de pago listo antes de la llamada) y guárdalo como
...
3. **Decide con el dueño**: Avanza, Ajustar o No ahora. Si toca precio o
   capacidad de forma grande, pásalo por `pvt-founder-agent` (8 criterios,
...
El repositorio es la única memoria. Nunca los dos construyendo la misma pieza a la vez.
...
| El filtro no se puede escribir o `Fit avatar` es dudoso | `pvt-avatar-agent` | Qué bloque necesita el perfil y a quién excluir |
| Pitch listo, falta la llamada | `pvt-closing-agent` | Pitch, objeción más probable, caso elegido, link de pago listo |
| Promesa o precio nuevo hay que declararlo afuera | `pvt-content-agent` | La promesa congelada y el vocabulario único |
| Mensajes o bio de captura mencionan la oferta | `pvt-setter-agent` | Qué frase cambió y dónde |
| Cambió un entregable o el límite | `pvt-consulting-agent` | El entregable y el límite por escrito |
| Un cliente pide más de lo que su capa entrega o hay renovación | `pvt-success-agent` | La capa siguiente o la de continuidad definida |
| Cambio grande de precio, capacidad o meta | `pvt-founder-agent` | La decisión para los 8 criterios y el precio actual del Documento de contexto |

## Lo que esta skill NO hace

- No conduce la llamada, no maneja objeciones en vivo ni cobra (`pvt-closing-agent`).
- No define el avatar ni sus dolores (`pvt-avatar-agent`); los consume.
- No escribe ganchos, carruseles ni la bio (`pvt-content-agent`).
- No arma el onboarding ni entrega (`pvt-consulting-agent`).
- No edita el Documento de contexto: el precio actual lo actualiza `pvt-founder-agent`.
