<!-- Private Build OS · pb-ventas · 2026-09-11 · github.com/pvt-build/privatebuild-os -->
...
# pb-ventas — Ventas: conducir la conversación hasta el sí
...
3. Si el hub no existe o Notion no está conectado: dile que escriba "arrancar private build os" (skill `pb-os`). Si prefiere seguir sin Notion, pregunta de a una, máximo cuatro:
...
4. **Degradación**: si una hermana no está instalada, haz la versión mínima de su parte en línea y di cuál instalar. Ejemplo: no hay precio ni piso escritos (`pb-oferta`) → antes de preparar la llamada fija con él, en dos preguntas, el precio completo y el número bajo el cual no cierra, y deja anotado que la oferta está sin construir.
...
|---|---|---|
| Cerró | Cliente activo | Cobro confirmado y fecha de inicio → pasa a `pb-entrega` |
...
Claude construye y registra; GPT audita (ChatGPT en un Proyecto "Private Build OS" con el conector de Notion, o Codex con estas skills). Notion es la única memoria. Nunca los dos construyendo la misma pieza a la vez. Decide el dueño del negocio.
...
| Los leads llegan fríos, sin dolor nombrado o sin saber quién decide | `pb-captura` | Las llamadas donde pasó + qué dato faltó validar antes de agendar |
| Seguimiento largo de quien no responde, no-show, nurturing | `pb-captura` | Lead, último resultado, fecha de recontacto |
| "Lo pienso" repetido porque no se entiende por qué funcionaría, precio sin anclaje, entregas de más | `pb-oferta` | Citas de las objeciones + bloque de la oferta que falta |
| Leads que no eran para ti se repiten | `pb-avatar` | Los perfiles y el motivo, agrupados |
| Frase textual de un lead o una objeción que se repite | `pb-contenido` | La cita y el ángulo |
| Cerró | `pb-entrega` | Qué se prometió, cómo paga, fecha de inicio, quién decide, qué dijo que faltaba para el 10 |
| Patrón en 🔁 Aprendizajes o hace falta capturar un resultado de cliente como prueba | `pb-success` | Las filas y la pieza de prueba que falta |
| El precio, la meta o el cuello cambiaron | `pb-foundations` | Qué hay que actualizar en el Documento de contexto |

## Lo que esta skill NO hace

- No decide la oferta, el precio ni el piso: los usa desde 02 · Oferta (`pb-oferta`).
- No califica, no agenda ni persigue a quien no responde: eso es `pb-captura`.
...
Seguimiento largo, no-show y nurturing: pásalos a `pb-captura`.
...
Si no tienes ningún caso todavía, la captura se hace en la sesión donde aparece el resultado, no al terminar el programa: pásalo a `pb-success`.
