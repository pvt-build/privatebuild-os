<!-- Private Build OS · pb-contenido · 2026-09-11 · github.com/pvt-build/privatebuild-os -->
...
# pb-contenido — Contenido: cómo llegan los correctos hasta ti
...
2. **Lee 03 · Avatar** (dolores en sus palabras, preguntas, a quién no le sirve).
   ...
   hablarle a todos) y recomienda `pb-avatar`; si sigue, pide solo el dolor
...
**Sin hub o sin Notion:** que escriba "arrancar private build os" (`pb-os`). Si
...
conector de Notion, o Codex con estas skills). Notion es la única memoria. Nunca
...
|---|---|---|
| No hay dolores en palabras del avatar ni filtro | `pb-avatar` | Las piezas que no pudieron anclarse y el dato que falta |
| La petición no tiene recurso ni promesa detrás | `pb-oferta` | Qué pide la gente en los mensajes y qué recurso haría falta |
| Llegan conversaciones y no hay dónde quedan | `pb-captura` | La palabra de cada pieza viva y el primer mensaje escrito |
| Una conversación de perfil pide precio o llamada | `pb-captura` → `pb-ventas` | El lead con su `Pieza de origen` |
| El cierre muestra algo que corrige otra área | `pb-success` | Filas en 🔁 Aprendizajes con `Área que corrige` |
| El ángulo choca con hacia dónde va el negocio | `pb-foundations` | El ángulo y la tensión con la meta a 90 días |
| No existe el hub ni el Documento de contexto | `pb-os` | Nada: se arranca desde ahí |
...
- Si no hay recurso: versión mínima ("Si te pasa, escríbeme [PALABRA] y te
  cuento cómo lo resolvería en tu caso") y handoff a `pb-oferta`.
...
4. **Sin recurso listo no se publica la petición del recurso.** Se usa la
   ...
   caso") y se despacha a `pb-oferta`.
5. **Cada petición tiene su primer mensaje escrito**: el que entrega lo
   ...
   Lo que pasa después es de `pb-captura`.
